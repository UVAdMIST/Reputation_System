...
def robust_avg(df_input):
    ...
    # for i in range(1):
        # select not na to robust average ^_^
...
def beta_repu(df_input, df_weight, ff = 1.0):
    ...
    for i in range(len(df_input)):
        ...
        for j in df_input:
            ...
            #         print '==========================='
            #         print c1_stats['mean'][i]
            #         print df_input[j][i]
            #         print dev
            #         print '@@@@@@@@@@@@@@@@@@@@@@@@@@@'
            # T = abs(dev / c1_stats['std'][i])
            T = dev / std_weight

            # num_notnull = df_input1.notnull().sum(axis=1).loc[i]

            ## if less than 3 stations, don't run the reputation system
            #if num_notnull <= 2.0:
            #    df_alpha.at[i + 1, j] = df_alpha[j][i]
            #    df_beta.at[i + 1, j] = df_beta[j][i]

            # if std = 0, do not change the score!
